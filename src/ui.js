...
export function createOrbitalPaths(scene, sunGroup, planetGroups) {
    // Remove existing orbital paths if any
    const existingPaths = scene.children.filter(child => child.userData.isOrbitalPath);
    existingPaths.forEach(path => scene.remove(path));
    
    // Recreate all orbital paths
    ...
    planetGroups.forEach(planet => {
        ...
        // Add metadata to identify it as an orbital path
        circle.userData = { 
            isOrbitalPath: true,
    ...
    console.log('Orbital paths created and ready for toggle');
}

// Toggle orbital paths on / off
export function toggleOrbitalPaths(scene) {
    // Find all orbital paths in the scene
    const orbitalPaths = scene.children.filter(child => child.userData && child.userData.isOrbitalPath);
    
    if (orbitalPaths.length === 0) {
        console.warn('No orbital paths found in scene');
        return;
    }
    
    // Check if paths are currently visible (based on the first one)
    const arePathsVisible = orbitalPaths[0].visible;
    ...
    console.log(`Orbital paths ${!arePathsVisible ? 'shown' : 'hidden'} (${orbitalPaths.length} paths)`);
}

...
///// HYPERSPACE VR EFFECTS /////

// Hyperspace streak effect variables
let vrStreakLines = [];
const vrStreakCount = 200; // More streaks for better immersion in VR
const vrStreakLength = 100; // Length of each streak
const vrMaxStreakSpeed = 300; // Maximum speed during acceleration
const vrMinStreakSpeed = 50; // Minimum speed at start and end
let vrCurrentStreakSpeed = vrMinStreakSpeed; // Current dynamic speed
const vrStreakColor = 0xd9fdff; // Light blue

/**
 * Creates hyperspace streak effects for VR environment
 * @param {THREE.Scene} scene - The scene to add streaks to
 * @param {THREE.Camera} camera - The VR camera
 * @returns {Array} - Array of streak line objects
 */
export function createHyperspaceStreaks(scene, camera) {
    console.log("Creating VR hyperspace streaks");
    
    // Clear existing streaks before creating new ones
    vrStreakLines.forEach(streak => {
    ...
    for (let i = 0; i < vrStreakCount; i++) {
        ...
        // Position streaks in a wider field of view for VR immersion
        const angle = Math.random() * Math.PI * 2;
        const radius = 5 + Math.random() * 20;
        
        positions[0] = Math.cos(angle) * radius; // Start X
        positions[1] = Math.sin(angle) * radius; // Start Y
        positions[2] = -20 - Math.random() * (vrStreakLength * streakScale); // Start Z (closer at beginning)
        positions[3] = positions[0]; // End X (same as start for now)
        positions[4] = positions[1]; // End Y
        positions[5] = positions[2] + (vrStreakLength * streakScale); // End Z (shorter length at start)
        ...
        vrStreakLines.push({ 
            line, 
            positions: positions,
            initialLength: vrStreakLength * streakScale,
            targetLength: vrStreakLength,
            currentLength: vrStreakLength * streakScale
        });
    }
    
    return vrStreakLines;
}

/**
 * Updates hyperspace streaks with dynamic speed for VR
 * @param {number} deltaTimeInSeconds - Time elapsed since last frame
 * @param {THREE.Camera} camera - The VR camera for positioning
 * @param {boolean} isActive - Whether hyperspace is active
 */
export function updateHyperspaceStreaks(deltaTimeInSeconds, camera, isActive) {
    // If hyperspace is active, gradually increase streak speed
    if (isActive && vrCurrentStreakSpeed < vrMaxStreakSpeed) {
        vrCurrentStreakSpeed += (vrMaxStreakSpeed - vrMinStreakSpeed) * 0.05;
    } 
    // If hyperspace is not active, gradually decrease streak speed
    else if (!isActive && vrCurrentStreakSpeed > vrMinStreakSpeed) {
        vrCurrentStreakSpeed -= (vrMaxStreakSpeed - vrMinStreakSpeed) * 0.1;
    }
    ...
    vrStreakLines.forEach((streak) => {
        ...
        if (isActive && streak.currentLength < streak.targetLength) {
            streak.currentLength += (streak.targetLength - streak.initialLength) * 0.05;
        } 
        // Reduce streak length when deactivating
        else if (!isActive && streak.currentLength > streak.initialLength) {
            streak.currentLength -= (streak.targetLength - streak.initialLength) * 0.1;
        }
        ...
        for (let i = 0; i < positions.length; i += 3) {
            positions[i + 2] += vrCurrentStreakSpeed * deltaTimeInSeconds;
        }

        // Reset streak if out of range
        if (positions[5] > 100) {
            // Get random angle for circular distribution around view direction
            const angle = Math.random() * Math.PI * 2;
            const radius = 5 + Math.random() * 20;

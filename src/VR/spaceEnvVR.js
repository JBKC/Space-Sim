...
const STAR_SIZE = 25;
const STAR_VISIBILITY_RADIUS = 30000; // New constant for visibility radius
...
// Update stars - now with complete visibility cutoff at STAR_VISIBILITY_RADIUS
export function updateStars(stars, cameraPosition) {
    ...
    // Update star visibility based on distance to camera
    for (let i = 0; i < STAR_COUNT * 3; i += 3) {
        ...
        // Create a "rolling sphere" that follows the spacecraft - rendering only stars in the immediate environment
        
        // Check if star is beyond the visibility radius
        if (distance > STAR_VISIBILITY_RADIUS) {
            // Make the star invisible by setting its color to zero
            colors[i] = 0;     // R
            colors[i + 1] = 0; // G
            colors[i + 2] = 0; // B
            
            // Respawn the star in a new random position if it's too far away from the camera
            if (distance > SPACE_RADIUS * 0.8) {
                const theta = Math.random() * Math.PI * 2;
                const phi = Math.acos(2 * Math.random() - 1);
                const radius = STAR_VISIBILITY_RADIUS * 0.8 * Math.pow(Math.random(), 1/3);
            ...
            // Star is within visibility radius, calculate brightness
            const minDistance = STAR_VISIBILITY_RADIUS * 0.3; // Stars closer than 30% of visibility radius are at full brightness
            let brightness = 1.0;
            
            if (distance > minDistance) {
                // Linear falloff from full brightness to zero at visibility radius
                brightness = 1.0 - (distance - minDistance) / (STAR_VISIBILITY_RADIUS - minDistance);
            }

...
export function updateStars(stars, cameraPosition, respawnRelativeToCamera = true) {
    ...
    for (let i = 0; i < STAR_COUNT * 3; i += 3) {
        ...
        if (distance > STAR_RANGE * 0.8) {
            if (respawnRelativeToCamera) {
                ...
                // Use a fixed world coordinate system for respawning
                // This keeps stars in fixed positions independent of camera rotation
                const theta = Math.random() * Math.PI * 2;
                const phi = Math.acos(2 * Math.random() - 1);
                const radius = STAR_RANGE * Math.pow(Math.random(), 1/3);

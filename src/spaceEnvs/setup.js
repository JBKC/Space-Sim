...
// Log detection
console.log("Quest browser detection:", isQuestBrowser);
console.log("navigator.xr available:", !!navigator.xr);

// Create VR button - customized for Quest if needed
let vrButton;

if (isQuestBrowser && !navigator.xr) {
    console.log("Creating special Quest-compatible VR button");
    
    // Create manual button for Quest browsers that don't report navigator.xr correctly
    vrButton = document.createElement('button');
    vrButton.id = 'quest-vr-button';
    ...
    // Add click handler for Quest browsers
    vrButton.addEventListener('click', () => {
        console.log('Quest VR Button clicked');
        
        // Force XR session flag on
        window.isInXRSession = true;
        if (typeof window.updateVRStatus === 'function') {
            window.updateVRStatus();
        }
        
        // Show help overlay for Quest VR mode
        const helpOverlay = document.createElement('div');
        helpOverlay.id = 'quest-vr-help';
        ...
        helpOverlay.innerHTML = `
            <div style="max-width: 80%; background: rgba(0, 102, 255, 0.5); padding: 20px; border-radius: 10px; border: 2px solid #00ccff;">
                <h2 style="color: #00ccff; margin-top: 0;">Quest VR Mode Instructions</h2>
                <p>This special VR mode creates a stereoscopic view for your Quest.</p>
                <p>You will see the scene in 3D, but head tracking is not available.</p>
                <p><b>Controls:</b></p>
                <ul style="text-align: left; display: inline-block;">
                    <li>Use your Quest controller joystick to steer</li>
                    <li>Press trigger to accelerate</li>
                    <li>Touch screen controls also work</li>
                </ul>
                <button id="continue-to-vr" style="
                    background: #00ccff;
                    color: black;
                    border: none;
                    padding: 12px 24px;
        ...
        document.getElementById('continue-to-vr').addEventListener('click', () => {
            helpOverlay.remove();
            
            // Add a clear message about entering VR mode
            console.log("QUEST VR MODE ACTIVATED - Starting special VR mode...");
            
            // Start XR animation loop - with more robust handling
            if (typeof window.initXRAnimationLoop === 'function') {
                try {
                    console.log("Calling initXRAnimationLoop...");
                    window.initXRAnimationLoop();
                    console.log("XR animation loop function called successfully");
                    
                    // Check if the animation loop is actually set
                    if (renderer.getAnimationLoop()) {
                        console.log("✅ Animation loop successfully set on renderer");
                    } else {
                        console.error("❌ Animation loop not set on renderer - trying to set directly");
                        try {
                            // Get the callback function directly from the window object
                            const xrCallback = window.initXRAnimationLoop._cachedCallback;
                            if (xrCallback && typeof xrCallback === 'function') {
                                renderer.setAnimationLoop(xrCallback);
                                console.log("Set animation loop directly using cached callback");
                            } else {
                                console.error("Could not find cached callback for direct animation loop setting");
                            }
                        } catch (err) {
                            console.error("Error trying to set animation loop directly:", err);
                        }
                    }
                } catch (err) {
                    console.error("Error starting XR animation loop:", err);
                }
            } else {
                console.error("initXRAnimationLoop function not found on window object!");
            }
            ...
            exitButton.addEventListener('click', () => {
                ...
                if (renderer && renderer.setAnimationLoop) {
                    ...
                    console.log("Stopped XR animation loop for Quest");
                }
                
                // Hide exit button, show enter button
                exitButton.remove();
    ...
    // Use standard Three.js VRButton
    vrButton = VRButton.createButton(renderer);
    
    // Add debug info to standard button
    vrButton.addEventListener('click', () => {
        console.log('VR Button clicked');
        // Log the XR object state
        if (navigator.xr) {
            console.log('navigator.xr is available');
            
            // Try to detect if we're on a Quest
            if (isQuestBrowser) {
                console.log('Quest browser detected, forcing WebXR mode');
            }
        } else {
            console.log('navigator.xr is NOT available');
        }
    });
}
...
renderer.xr.addEventListener('sessionstart', () => {
    ...
    // Update global XR session flag
    window.isInXRSession = true;
    
    // Update VR status indicator if it exists
    if (typeof window.updateVRStatus === 'function') {
        window.updateVRStatus();
    }
    
    // Import the initXRAnimationLoop function from main.js
    if (typeof window.initXRAnimationLoop === 'function') {
        console.log('Initializing XR animation loop');
        window.initXRAnimationLoop();
    } else {
        console.warn('initXRAnimationLoop function not found');
    }
});
...
renderer.xr.addEventListener('sessionend', () => {
    ...
    // When XR session ends, stop the XR animation loop
    if (renderer && renderer.setAnimationLoop) {
        console.log('Stopping XR animation loop');
        renderer.setAnimationLoop(null);
    }
});
